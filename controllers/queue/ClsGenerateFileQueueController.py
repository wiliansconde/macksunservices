...
from repositories.queue.ClsGenerateFileQueueRepository import ClsGenerateFileQueueRepository


class ClsGenerateFileQueueController:

    @staticmethod
    def enqueue_generation_request(instrument: ClsInstrumentEnum, resolution: ClsResolutionEnum, target_date: datetime):
        ClsGenerateFileQueueRepository.insert_if_not_exists(instrument, resolution, target_date)

    @staticmethod
    def get_next_pending_request():
        return ClsGenerateFileQueueRepository.get_next_pending_request()

    @staticmethod
    def update_status_completed(request_id):
        ClsGenerateFileQueueRepository.update_status(request_id, "COMPLETED")

    @staticmethod
    def update_status_failed(request_id, error_message=""):
        ClsGenerateFileQueueRepository.update_status(request_id, "FAILED", error_message)
    ...
    def populate_queue_from_stats():
        ...
        """
        stats_list = ClsGenerateFileQueueRepository.get_all_stats()
